...
def main():
    ...
    num_transactions = int(input(f'{bcolors.BLUE}How many transactions? {bcolors.ENDC}')
                           or 1)
    all_events = {}
    for i in range(num_transactions):
        ...
        is_even_split = input(f'\t{bcolors.YELLOW}Even split for this transction?(y/n): {bcolors.ENDC}').lower()
        if is_even_split == 'y':
            ...
            for id in ids_involved:
                price_per_person = calculate_price_per_person(subtotal, num_ids, people, id)
                people[id].add_purchase(event, purchase, price_per_person, tax_perc, tip_perc)
        else: # transaction is not evenly split
            print(f'\t\t{bcolors.PURPLE}Enter all purchases. Enter DONE when finished.{bcolors.ENDC}')
            while True:
                purchase_details = input(f'\t\t{bcolors.PURPLE}Purchase, price, and name(s)?{bcolors.ENDC} ').split()
                if purchase_details[0] == 'DONE': break
                ...
                if len(purchasers) > 1:
                    is_ppp = input(f'\t\t\t{bcolors.PURPLE}Is this price per person?(y/n){bcolors.ENDC} ')
                    if is_ppp == 'n':
                        ...
                        if split_purchasers:
                            for split_purchaser in split_purchasers:
                                if split_purchaser == p.name:
                                    p.add_purchase(event, '1/2 ' + purchase, price / len(split_purchasers), tax_perc, tip_perc)
                                    found.add(split_purchaser)
                        else:
                            if purchaser == p.name:
                                p.add_purchase(event, purchase, price, tax_perc, tip_perc)
                                found.add(purchaser)
                                break
                not_found = set(purchasers) - found
                not_found = ' '.join(list(not_found))
                if not_found:
                    print(f'\t\t\t{bcolors.FAIL}Could not find {not_found}{bcolors.ENDC}')

            # TODO: convert into func
            check_subtotal = sum([person.purchases[event]['total'] for person in people if event in person.purchases])
            if check_subtotal < subtotal + 0.01 and check_subtotal > subtotal - 0.01:
                print(f'{bcolors.GREEN}SUBTOTAL FOR {event.upper()} MATCHES RECEIPT{bcolors.ENDC}')
            else:
                if check_subtotal < subtotal:
                    print(f'\t{bcolors.FAIL}Missing purchases for {event}. Subtotal={subtotal}, found={check_subtotal}{bcolors.ENDC}')
                elif check_subtotal > subtotal:
                    print(f'\t{bcolors.FAIL}Too many purchases / incorrect price for purchase(s). Subtotal={subtotal}, found={check_subtotal}{bcolors.ENDC}')
    ...
    for i, event in enumerate(sorted_events):
        if i % 2 == 0:
            line += f'{bcolors.ORANGE}{all_events[event]["tax"] * 100}%\t{bcolors.ENDC}'
        else:
    ...
    for i, event in enumerate(sorted_events):
        if i % 2 == 0:
            line += f'{bcolors.ORANGE}{all_events[event]["tip"]:.2f}%\t{bcolors.ENDC}'
        else:
    ...
    for person in people:
        ...
        for event in sorted_events:
            ...
            num_events += 1
        orders += ')'
        line += (f'{bcolors.GREEN}'
                 f'${person.total}'
                 f'{bcolors.ENDC}')
        print(f'{line}{bcolors.PURPLE} {orders}{bcolors.ENDC}')

def get_people():
    return [Person(person) for person in sorted(input(f'{bcolors.BLUE}List of people: {bcolors.ENDC}').split())]
...
def prompt_people_menu(event, people):
    ...
    return [int(id) - 1
            for id in input(f'{bcolors.YELLOW}'
                            f'\tPeople Menu\n'
                            f'\t{temp}'
                            f'{prompt}'
                            f'{bcolors.ENDC}')
...
def calculate_num_ids(ids_involved, people):
    ...
    for id in ids_involved:
        count += len(people[id].name.split('+')) # ok even if the name does not have a '+'
    return count
...
if __name__ == '__main__':
    print(f"{bcolors.UNDERLINE}{bcolors.PINK}Hello, This is a tool to split receipts.{bcolors.ENDC}")
    main()
